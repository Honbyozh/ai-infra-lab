...
>
> 范围：本文只记录承担代码运行、模型服务、容器和性能实验的主实验机。另一台学习机用于源码阅读与文档记录，不纳入本文基线；两边的学习资料后续手工合并。
>
...
当前最重要的三个阻塞项是：确认是否需要接入 NVIDIA GPU、安装并验证容器运行时、建立独立且锁定版本的 Python 环境。它们完成后再开始 Day 1。
...
- [ ] 已确认宿主机是否具备 NVIDIA GPU，并选定 GPU 或无 GPU 路线。
